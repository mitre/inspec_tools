...
module Util
  ...
  class Trans < Parslet::Transform
    ...
    rule(:header => simple(:header), :applicability => sequence(:applicability), :description => sequence(:description), :rationale => sequence(:rationale)) { {:title => header.to_s , 
           :level => applicability[0].to_s, :descr => description[0].to_s + rationale[0].to_s} }
  end
