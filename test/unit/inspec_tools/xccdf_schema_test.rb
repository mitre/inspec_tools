...
describe 'Validate conformant XCCDF' do
  ...
  # These tests are intended only for Mac due to native xmllint executable usage that validates XML is conformant to schema
  let(:supported_os) { /darwin(.+)$/.match?(::RbConfig::CONFIG['host_os']) }
  ...
  describe 'when XCCDF version is 1.1 and conformant' do
    ...
    it 'produces XML that is valid' do
      ...
      skip 'Unsupported OS for this test' unless supported_os
